...
export async function getTwoHeroesSolution(heroData, hero1Id, hero2Id) {
  ...
  return new Promise (((resolve, reject) => {
    let hero1 = '';
    let hero2 = '';
    heroData.forEach(function(h) {
      if(h.id === hero1Id){
        hero1 = h;  
      }
      else if (h.id === hero2Id){
        hero2 = h;
      }
    });
    if ((hero1 || hero2) === undefined){
      reject();
    }
    else {
      let bothHero = {
  ...
  */
  ...
  let heroDuo = await Promise.all([getHeroByIdAsync(heroData, hero1Id), getHeroByIdAsync(heroData, hero2Id)]);
